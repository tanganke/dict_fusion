...
from ..tasks import check_parameterNamesMatch
...
def _get_block_wise_weights_by_partitions(
    weights: Tensor, num_partitions: int, num_models: int
) -> Tensor:
    """
...
def get_block_wise_weights_by_partitions(
...
def _get_block_wise_weights_by_block_size(
    weights: Tensor, block_size: int, num_models: int, init_value: float = None
) -> Tensor:
    """
    Returns a tensor of block-wise weights for a given tensor of weights.
...
def get_block_wise_weights_by_block_size(
...
def _fuse_weights(block_wise_weights: Tensor, model_weights: List[Tensor]) -> Tensor:
    ...
    """
    assert (
        block_wise_weights.dim() == 2
    ), f"block_wise_weights must be a 2D tensor, but got {block_wise_weights.dim()}D"
    ...
    if num_partitions == 1:
        return sum(
            block_wise_weights[i, 0] * model_weights[i].to(block_wise_weights.device)
        ...
        for i in range(num_models):
            for j in range(num_partitions):
                ans[block_size * j : block_size * (j + 1)] += block_wise_weights[
                    i, j
                ] * model_weights[i][block_size * j : block_size * (j + 1)].to(
