    ...
    def ties_merging(self):
        ...
        tv_flat_checks = torch.vstack(
            [
                ...
                for check in track(self.task_vectors, "flattening task vectors")
            ]
        )

        K = 20
        for scaling_coef in tqdm(np.linspace(0, 1, 11), "scaling_coef"):
            merged_tv = ties_merging(
                tv_flat_checks, reset_thresh=K, merge_func="dis-sum"
            )
