...
def main(cfg: DictConfig):
    ...
    logger = TensorBoardLogger(
        LOG_DIR / cfg.model.name / cfg.dataset.name, name=cfg.peft.name
    )
        ...
        if (cache_dir := CACHE_DIR / "datasets" / cfg.dataset.name).exists():
            ...
            if hasattr(cfg.dataset, "preprocessor"):
                if hasattr(cfg.dataset.preprocessor, "template_file"):
                    cfg.dataset.preprocessor.template_file = str(
                        TEMPLATE_DIR / cfg.dataset.preprocessor.template_file
                    )
                ...
                datasets = datasets.map(
                    preprocessor,
                    **cfg.dataset.map_kwargs
                    if hasattr(cfg.dataset, "map_kwargs")
                    else {"batched": True},
                )
